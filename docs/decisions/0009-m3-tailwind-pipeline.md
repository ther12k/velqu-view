...
generated sheet joins the cascade **after every author sheet** — so
utilities beat element rules, lose to inline styles, and normal CSS
order/specificity rules decide everything else. Disabling the pipeline
(or never enabling it) changes nothing: every pre-existing fixture
passed unmodified.
...
text-align, leading), borders (widths, solid/none, gray-200 default
color, per-side colors), rounded, and overflow.
